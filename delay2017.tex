...
We demonstrate the utility of these extensions by mapping an optimal delay system onto various spiking dynamical networks using higher-order models of the synapse.
We show that this system nonlinearly encodes its input history, using a scale-invariant representation, with accuracy depending on the frequency content of the input signal.
...
How these low-level mechanisms interact in order to encode information about a stimulus, across time, is the subject of considerable study.
...
The flexibility of the NEF has lead to it being deployed on mixed-analog-digital neuromorphic chips~\citep{choudhary2012silicon, corradi2014, voelker2017iscas} and digital architectures~\citep{bekolay2013nengo, wang2014compact, mundy2015efficient}.
...
Similarly, the synapse models commonly used in biological models incorporate distinct rise- and fall-times due to separate time-courses of transmitter binding and unbinding, as well as axonal transmission delays due to the finite-velocity propagation of action potentials~\citep{roth2009modeling}.
...
A particularly important dynamical system that the NEF has not addressed is the continuous-time delay.
This system must represent a rolling window of input history.
Here, we provide a novel derivation of a low-dimensional linear approximation to a continuous-time delay, and use this to nonlinearly encode the history of the network's input up to some finite length.
This \emph{delay network} is scale-invariant, with a level of accuracy that depends on the input frequency, chosen dimensionality (i.e.,~the order of the approximation), and particular synapse model.
...
In section~\ref{sec:delay-implementation}, we use the NEF to derive a spiking dynamical network that delays its input signal by some fixed length of time, and then analyze this network in section~\ref{sec:temporal-coding} to show that it nonlinearly encodes the stimulus across the entire delay interval.
...
In section~\ref{sec:results}, we exploit the methods from section~\ref{sec:extensions} to demonstrate their utility in calculating delays and to provide insights into possible neural mechanisms.
...
In this section, we provide an overview of these methods applied to training both feed-forward and recurrent connection weights, in order to implement linear dynamical systems (although these methods extend to nonlinear dynamical systems as well).
We present this framework in a manner that is consistent with the Nengo~2.3.1 simulator~\citep{bekolay2013nengo}, which implements the NEF among other approaches.
...
Then $a_i^\vec{x}(t) = \sum_m \delta \left(t - t_{i\text{,}m} \right)$, where $\delta(\cdot)$ is the Dirac delta, and $\left\{ t_{i\text{,}m} \right\}$ is a sequence of spike-times given by equation~\ref{eq:encoding}. % via $G_i \left[ \cdot \right]$.
...
For now, we fix $h(t) = \frac{1}{\tau} e^{-\frac{t}{\tau}}$, which is the canonical first-order lowpass filter (also known as a leaky integrator).
This is the conventional choice of synapse in the NEF, since it strikes a convenient balance between mathematical simplicity and biological plausibility.
...
Let $D = \transpose{\left[ \vec{d}_1\text{,}\, \ldots\text{,}\, \vec{d}_n \right]} \in \mathbb{R}^{n \times q}$ be the decoder matrix that decodes $\vec{x}(t)$ from the population's activities $\left( a_i^\vec{x}(t) \right)$ at time $t$.
...
Equation~\ref{eq:rep-decode} takes a linear combination of the filtered activities, in order to recover a filtered version of the encoded signal.
...
To determine these decoders, we first let $r_i(\vec{v})$ be the limiting average firing rate of the $i^{\text{th}}$ neuron under the constant input $\vec{v} \in \mathbb{R}^q$:
...
Furthermore, this is a regularized least-squares optimization problem, which may be solved by uniformly sampling $S$~\citep{voelker2017} and applying a standard regularized least-squares solver to the sampled data~\citep{bekolay2013nengo}.
...
This zeroth-order approximation clearly holds in the steady-state for constant $\vec{x}(t)$, and turns out to be ideal in practice for low-frequency $\vec{x}(t)$, and likewise for $h(t)$ that filter out high frequencies (i.e.,~when the synaptic time-constant $\tau$ is large).
...
This, in turn, permits us to perform useful computations by transforming the state-vector with $\mathcal{O} \left( \frac{1}{\sqrt{n}} \right)$ error in the presence of neural spiking~\citep[][p.~47]{eliasmith2003neural}.
...
Principle~3 is a means of harnessing the dynamics of the synapse model for network-level information processing.
...
\end{split}
\end{equation}
where the time-varying signal $\vec{x}(t)$ represents the system state, $\dot{\vec{x}}(t)$ its time-derivative, $\vec{y}(t)$ the output, $\vec{u}(t)$ the input, and the time-invariant matrices $(A\text{,}\, B\text{,}\, C\text{,}\, D)$ fully describe the system.
...
However, given our description of the first two principles, the dynamical primitive at our disposal is the \emph{leaky} integrator, given by our canonical first-order lowpass filter modeling the synapse:
...
For comparison, the Laplace transform of the integrator is $\frac{1}{s}$.}

Principle~3 of the NEF states that in order to implement equation~\ref{eq:lti} in a population of neurons that represents $\vec{x}(t)$, we must compensate for the effect of ``replacing'' the integrator with a leaky integrator, by driving the synapse with $\tau \dot{\vec{x}}(t) + \vec{x}(t)$ (instead of $\dot{\vec{x}}(t)$).
This can be achieved as follows: implement the recurrent transformation $(\tau A + I)\vec{x}(t)$, and the input transformation $\tau B \vec{u}(t)$, but use the same output transformation $C\vec{x}(t)$, and the same passthrough transformation $D\vec{u}(t)$~\citep[][pp.~221--225]{eliasmith2003neural}.
Each of these transformations may be implemented using Principle~2.
...
\begin{figure}[H]
  ...
  \caption{ \label{fig:lti-system-mapped}
    ...
    The lowpass is driven by the signal $\tau \dot{\vec{x}}(t) + \vec{x}(t)$.
  }
\end{figure}

The correctness of this mapping relies on three assumptions: (1)~the synapse model is equation~\ref{eq:lowpass}, (2)~the network is simulated in continuous time (or the discrete time-step is sufficiently small), and (3)~the necessary representations and transformations are sufficiently accurate, such that the approximation error $\mathcal{O} \left( \frac{1}{\sqrt{n}} \right)$ from equation~\ref{eq:trans-decode} is negligible.
In other words, assuming $n$ is sufficiently large, Figure~\ref{fig:lti-system-mapped} is just an equivalent way of expressing Figure~\ref{fig:lti-system}, but using the synapse model instead of the integrator as the dynamical primitive.
...
Principle~3 is useful for accurately implementing a wide class of dynamical systems (e.g.,~integrators, oscillators, attractor networks, etc.) to solve specific problems that frequently arise in neural modeling~\citep{eliasmith2000b, singh2004, eliasmith2005b, singh2006}.
...
Furthermore, the class of state-space models is isomorphic to the class of all finite-dimensional causal linear filters, or equivalently all rational (finite-order) proper transfer functions, which is a large and useful class of dynamical systems employed widely in control applications.
Principle~3 also naturally generalizes to nonlinear dynamical systems, but this is outside the scope of this work; we refer the reader to \citet{voelker2017iscas} for a recent nonlinear extension.

\section{Spiking Delay Networks}
\label{sec:delay}

A fundamental dynamical system that has not yet been discussed within the NEF literature is the continuous-time delay of $\theta > 0$ seconds,\footnote{
...
y(t) = (u \ast \delta_{-\theta})(t) = u(t - \theta) \text{,}
\end{align}
...
The task of computing this function both accurately and efficiently in a spiking dynamical network is a significant theoretical challenge, that, to our knowledge, has previously remained unsolved.

The continuous-time delay is worthy of detailed consideration for several reasons.
First, it is non-trivial to implement using continuous-time dynamical primitives.
...
For instance, the power spectrum of a signal may be expressed as a nonlinear combination of delays, as may any finite impulse response~(FIR) filter.
...
As it is impossible in practice (i.e.,~given finite-order continuous-time resources) to implement an arbitrary delay, we instead approximate $u(t)$ as a low-frequency signal in the \emph{Laplace domain}, or, equivalently, approximate equation~\ref{eq:time-delay} as a low-dimensional system expanded about the zeroth frequency.
We begin by converting the system into the Laplace domain, $\mathcal{L} \left\{ y(t) \right\} = \mathcal{L} \left\{ u(t) \right\} \mathcal{L} \left\{ \delta_{-\theta}(t) \right\}$, and then using the fact that $\mathcal{L} \left\{ \delta_{-\theta}(t) \right\} = e^{-\theta s}$ to obtain:
...
Equation~\ref{eq:tf-delay} should be understood as an equivalent way of expressing equation~\ref{eq:time-delay} in the Laplace domain, where the variable $s$ denotes a complex value in the frequency domain. %while $t$ is non-negative in the time domain.
% Unimportant reminder? The transfer function is the Laplace transform of system's impulse response.

The state-space model (equation~\ref{eq:lti}) of a LTI system is related to its transfer function by the following:
\begin{align} \label{eq:ss2tf}
F(s) = C(sI - A)^{-1}B + D \text{.}
\end{align}
Conversely, a transfer function can be converted into a state-space model satisfying equation~\ref{eq:ss2tf} if \emph{and only if} it can be written as a proper ratio of finite polynomials in $s$.
...
\begin{aligned}
\left[ p/q \right] e^{-\theta s} &= \frac{\mathcal{B}_{p}(-\theta s)}{\mathcal{B}_{q}(\theta s)} + \mathcal{O}(s^{p + q + 1}) \text{,} \\
...
However, na\"ively applying this conversion leads to representational issues, due in part to the factorials in equation~\ref{eq:pade}.
...
This model is now equivalent to equation~\ref{eq:pade}, but without factorials, and in the form of equation~\ref{eq:lti}.\footnote{
...
In Figure~\ref{fig:delay-example}, we implement a $1$\,s delay of $1\,$Hz band-limited white noise using $\num{1000}$ LIF neurons in a $6$-dimensional spiking dynamical network.
The normalized root-mean-squared error (NRMSE) of the output signal is $0.043$ (normalized so that $1.0$ would correspond to random chance).
...
\begin{figure}[H]
  ...
  \caption{ \label{fig:delay-example}
    ...
    We filter the activities (and $u(t - \theta)$) with a lowpass ($\tau=0.02$\,s).
    The plotted spikes are filtered with $\tau=0.1$\,s, and encoded with respect to $\num{1000}$ encoders sampled uniformly from the surface of the hypersphere (sorted by time to peak activation).
  }
\end{figure}
...
The state-vector of the delay network represents a rolling window of length $\theta$.
...
In Figure~\ref{fig:delay-full}, we take different linear transformations of the same state-vector, according to equation~\ref{eq:delay-readouts}, to flexibly decode delays between $0$ and $\theta$ from the same neural activity.\footnote{
The least-squares problem from equation~\ref{eq:decoder_solution} need only be solved once to decode $\vec{x}(t)$ from the neural activity.
...
Importantly, this state-vector is simultaneously encoded by heterogeneous neural nonlinearities, which supports the decoding of nonlinear functions across the entire window (i.e.,~functions that we can compute using Principles~1 and~2).
...
\begin{figure}[H]
  ...
  \caption{ \label{fig:delay-full}
    ...
    (Right)~Example simulation given white noise input with a cutoff frequency of $\theta^{-1}$\,Hz.
    % See appendix~\ref{app:window} for details.
  }
\end{figure}

In Figure~\ref{fig:basis-functions}, we use equation~\ref{eq:delay-readouts} to visualize the temporal basis functions of the state-space.
Each basis function corresponds to the continuous window of history represented by a single dimension of the delay network.
...
This allows us to visualize a subset of the state-vector trajectories in three dimensions, via projection onto their first three principal components (see Figure~\ref{fig:pca}-Top). %\footnote{
...
This means that other delays will simply stretch or compress the impulse response linearly in time.
...
To determine these functions for a wide class of synapses, we now proceed to develop theory that extends the NEF to harness more sophisticated synapse models.
...
We focus on building a comprehensive theory for linear systems, but many of these same techniques carry over to the case of nonlinear dynamical systems as well~\citep{voelker2017iscas}.

Let $F(s)$ be the transfer function for the linear dynamics that we wish to implement (equations~\ref{eq:lti} and~\ref{eq:ss2tf}), and let $H(s)$ be the transfer function for an arbitrary linear synapse model.
As stated in section~\ref{sec:principle3}, introducing the synapse model means replacing the integrator ($s^{-1}$) with $H(s)$.
Notably, substituting $H(s)$ for $s^{-1}$ results in the transfer function $F \left( H(s)^{-1} \right)$, which no longer implements the original, desired dynamics $F(s)$. %, via the change-of-variables $s \longleftrightarrow H(s)^{-1}$.
...
\end{equation}
will implement the desired dynamics when using $H(s)$ as the dynamical primitive, by equation~\ref{eq:maps-onto} (see Figure~\ref{fig:lti-system-mapped-general}).
...
For instance, it includes the first-order lowpass synapse that is standard in the NEF.
...
In addition, equation~\ref{eq:synapse} includes a generalization of the alpha synapse, the double-exponential synapse, $\frac{1}{(\tau_1 s + 1)(\tau_2 s + 1)}$~\citep{wilson1989simulation}---the convolution of two exponentials with time-constants $\tau_1$ and $\tau_2$---which has different rise- and fall-times to account for the separate time-courses of rapid transmitter binding followed by slow unbinding~\citep{destexhe1994synthesis, hausser1997estimating, roth2009modeling}.
...
Furthermore, this class contains more exotic models, such as the $Q$-bandpass synapse model, $\frac{1}{\frac{1}{\omega^2}s^2 + \frac{1}{\omega Q}s + 1}$, where $\omega$ is the peak frequency in radians per second, and $Q$ is inversely proportional to the bandwidth.
...
Since $s^j$ is the $j^{\text{th}}$-order differential operator, this form of $B^H$ states that we must supply the $j^{\text{th}}$-order input derivatives $\vec{u}^{(j)}(t)$, for all $j = 1 \ldots k - 1$.
...
Then equation~\ref{eq:general-linear} shows that the ideal state-space model must implement the input transformation as a linear combination of input derivatives, $\sum_{j=0}^{k-1} B^H_j \vec{u}^{(j)}(t)$.
...
However, if the required derivatives are not included in the neural representation, then it is natural to use a ZOH method by assuming $\vec{u}^{(j)}(t) = 0$, for all $j = 1 \ldots k - 1$:
...
All networks were built and simulated using Nengo~2.3.1~\citep{bekolay2013nengo}, a Python tool for simulating large-scale neural networks.
...
Figures were created using Seaborn~\citep{michael_waskom_2015_19108}.
Everything is reproducible via: \path{https://github.com/arvoelke/delay2017}.

\subsection{Delay Networks with Higher-order Synapses}
\label{sec:pure_delay}

We begin by making the practical point that it is crucial to account for the effect of the simulation time-step in digital simulations, if the time-step is not sufficiently small relative to the time-course of the desired network-level dynamics.
...
First, it allows those deploying the NEF on neuromorphics to improve the accuracy of their systems given the higher-order dynamics of mixed-analog-digital synapses~\citep{voelker2017iscas}.
...
This same delay network accurately represents the history of an input stimulus across the delay interval by compressing it into a low-dimensional state, with length scaling as $\mathcal{O}\left( \frac{q}{f} \right)$, where $f$ is the input frequency.
...
While the error in our transformations scale as $\mathcal{O} \left( \frac{1}{\sqrt{n}} \right)$ due to independent spiking, it has been shown that instantaneous (unfiltered) feedback may be used to harness these spikes collaboratively and scale the error as $\mathcal{O} \left( \frac{1}{n} \right)$~\citep{boerlin2013predictive, thalmeier2016learning}.
This reduction in error would have dramatic consequences for the efficiency and scalability of neuromorphics by reducing total spike traffic~\citep{boahen2017neuromorph}.
However, it is currently unclear whether this approach can be applied to a more physically plausible setting while retaining this linear scaling property.
...
The identification of this control factor is connected to a more general property of the Laplace transform, $F \left( a^{-1} s \right) = \mathcal{L} \left\{ a f(at) \right\}$ for all $a > 0$, that we can exploit to modulate the width of a filter on-the-fly (in this case affecting the amount of delay; results not shown).
...
To compute these other delays, we would like to optimally approximate $e^{-\theta' s}$ with a transfer function $F_{\theta \rightarrow \theta'}(s) := \frac{\mathcal{C}(s; \, \theta, \theta')}{\mathcal{D}(s; \, \theta)} + \mathcal{O}(s^{p + q + 1})$ of order $[p / q]$, such that the denominator $\mathcal{D}(s; \, \theta)$ (which provides us with the recurrent transformation up to a change of basis) depends only on $\theta$, while the numerator $\mathcal{C}(s; \, \theta, \theta')$ (which provides us with the output transformation up to a change of basis) depends on some relationship between $\theta'$ and $\theta$.
...
\begin{align*}
&& e^{-\theta' s} &= \sum_{i=0}^\infty \frac{(-\theta' s)^i}{i !} = \frac{\mathcal{C}(s; \, \theta, \theta')}{\mathcal{D}(s; \, \theta)} + \mathcal{O}(s^{p + q + 1}) & \\
&& \iff \quad \mathcal{C}(s; \, \theta, \theta') &= \left( \sum_{i=0}^\infty \frac{(-\theta' s)^i}{i !} \right) \left( \sum_{j=0}^q d_j(\theta) s^j \right) + \mathcal{O}(s^{p + q + 1}) \text{.}
\end{align*}
...
We remark that $c_i(\theta, \theta) = \begin{hpmatrix}p \\ i\end{hpmatrix} \frac{(p + q - i)!}{(p + q)!} (-\theta)^i$, since $F_{\theta \rightarrow \theta}(s) = e^{-\theta s} + \mathcal{O}(s^{p + q + 1})$, by uniqueness of the Pad\'e approximants, and by equation~\ref{eq:pade}.
...
In general, one may consider any invertible matrix $T$ with the same shape as $A$, and observe that the state-space model $(TAT^{-1}\text{,}\, TB\text{,}\, CT^{-1}\text{,}\, D)$ corresponds to the same transfer function as $(A\text{,}\, B\text{,}\, C\text{,}\, D)$.
Thus, the state-space model is only unique up to a change-of-basis.
However, in the NEF, the basis $T$ may be `absorbed' into the representation of $\vec{x}(t)$ by using the encoders $ET$ in Principle~1 (which, in turn, results in the decoders $T^{-1}D^\vec{f}$ from Principle~2).
In other words, considering an alternative state-space model is equivalent to considering a change of basis for the encoding.
...
We balance the range of values by numerically computing the $T$ that results in a ``balanced realization'' of $(A\text{,}\, B\text{,}\, C\text{,}\, D)$~\citep{laub1987computation}.
...
Finally, we scale each dimension by a diagonal transformation $T$ with the $i^{\text{th}}$ diagonal equaling $\max_t \left| x_i(t) \right|$ where $x_i(t)$ is obtained by simulating the desired system directly on a randomly sampled input.
...
If we make the ZOH assumption---that the input signal is held piecewise-constant over each continuous-time interval of length $dt$---then $\vec{u}^{(j)}(t) = 0$ for all $j \ge 1$.
